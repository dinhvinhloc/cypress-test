describe('API002g1 - Get the stores nearby within a distance from a reference point', function () {
    it('Business Category: should allow you to get the stores nearby within a distance from a reference point', function () {
        var email = Cypress.env('merchant3');
        var password = Cypress.env('merchant3Pass');
        ...
        cy.waitUntil(()=> cy.getStoresAround(x,y,100).then((stores)=>{
            
            let IDList = stores.map(a => a.id);
            expect(IDList).to.not.contain(storeID);

        })); 

        email = Cypress.env('merchant3');
        password = Cypress.env('merchant3Pass');
        cy.Login(email, password);
