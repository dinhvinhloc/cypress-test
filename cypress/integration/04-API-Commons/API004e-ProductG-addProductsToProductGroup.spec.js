describe('API004e - Add Product to Product Group using API', function () {
    it('Business Category: should allow you to add Products to Prodduct Group', function () {
        ...
        // Creating a product with wrong format name and desc, 0 stores.
        cy.waitUntil(() => cy.get('@productInfo1').then((info)=>{ 
            info.name = "Test Product";
            info.description = "Test Product Description";
            info.stores = [];
            // No Product Groups attached to it.
            info.groups = [];
            productInfo1 = info;
        }));

        cy.waitUntil(()=> cy.createProductClone(productInfo1).then((response)=>{
            console.log(response);
            expect(response).to.not.have.key('id');
            expect(response).to.have.property('status', 400);
            expect(response.message).to.contain("must not be empty");
            })        
        ); 
    });
})
