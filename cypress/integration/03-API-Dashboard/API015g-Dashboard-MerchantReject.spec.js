describe("API015g - Redeemed Orders in the Sales Dashboard Orders Chart", () => {
    it("Business Category: On Merchant Rejecting the order the total redeemed orders in the order chart should be changed", () => {
        ...
        var refundO, refundOWithStore, updatedRefundO, updatedRefundOWithStore;
        ...
        cy.waitUntil(() => cy.getDashboardInit(startDate, endDate, merchantId).then((response) => {
            ...
            refundO = metrics.filter((el) => { return el.metric == 'TOTAL_REFUNDED_ORDERS' })[0].total;
            console.log(`Refund Orders:: ${refundO}`);
        }));
        ...
        cy.waitUntil(() => cy.getDashboardInitWithStore(startDate, endDate,merchantId, storeId).then((response) => {
            let metrics = response;
            refundOWithStore = metrics.filter((el) => { return el.metric == 'TOTAL_REFUNDED_ORDERS' })[0].total;
            console.log(`Refund Orders With Store:: ${refundOWithStore}`);
        }));
        ...
        // REJECT the order
        cy.waitUntil(() => cy.merchantOrderCommand(order.id, store1.storeID.id,"reject",{ rejectMessage: 'The store is closed'}).then((response)=>{
            console.log(response);
        ...
        cy.waitUntil(() => cy.getDashboardInit(startDate, endDate, merchantId).then((response) => {
            let metrics = response;
            updatedRefundO = metrics.filter((el) => { return el.metric == 'TOTAL_REFUNDED_ORDERS' })[0].total;
            console.log(`Updated Refund Orders:: ${updatedRefundO}`);
            expect(refundO+1).to.equal(updatedRefundO);
        }));

        //Checks the updated total sales with store
        cy.waitUntil(() => cy.getDashboardInitWithStore(startDate, endDate, merchantId, storeId).then((response) => {
            let metrics = response;
            updatedRefundOWithStore = metrics.filter((el) => { return el.metric == 'TOTAL_REFUNDED_ORDERS' })[0].total;
            console.log(`Updated Refund Orders with Store:: ${updatedRefundOWithStore}`);
            expect(refundOWithStore+1).to.equal(updatedRefundOWithStore);
        }));
