describe('017e - Edit Promotion - Percent Discount on Selected Items', function () 
{
    it('Percent Discount on Selected Items', function()
    {
        // Login
        ...
        cy.get('[data-cy="top-input-search"]').clear({force : true}).type('Percent Discount on Selected Items');
        ...
        cy.get('[data-cy="promotion-name"]').clear({force : true}).type('% Discount Selected Items Edit');
        ...
        cy.get('[data-cy="top-input-search"]').clear({force : true}).type('% Discount Selected Items Edit');
        cy.wait(1000);

        cy.get('.v-switch-label:first').should('contain','ACTIVE');

        // Check new name
        cy.get('[data-cy$=title]:first').should('contain','% Discount Selected Items Edit');
