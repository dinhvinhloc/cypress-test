describe('017d - Edit Promotion Buy one get one', function () 
{
    it('Edit Buy one get one', function()
    {
        // Login
        ...
        cy.get('[data-cy="top-input-search"]').clear({force : true}).type('Buy one Get one Free');
        ...
        cy.get('[data-cy="promotion-name"]').clear({force : true}).type('Buy one Get one Free Edit');
        ...
        cy.get('[data-cy="top-input-search"]').clear({force : true}).type('Buy one Get one Free Edit');
        cy.wait(1000);

        cy.get('.v-switch-label:first').should('contain','ACTIVE');

        // Check new name
        cy.get('[data-cy$=title]:first').should('contain','Buy one Get one Free Edit');
