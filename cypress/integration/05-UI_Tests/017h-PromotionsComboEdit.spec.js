describe('017h - Edit Promotion - Combo Deal', function () 
{
    it('Combo Deal', function()
    {
        ...
        cy.get('[data-cy="top-input-search"]').clear({force : true}).type('Combo Deal');
        ...
        cy.get('[data-cy="promotion-name"]').clear({force : true}).type('Combo Deal Edit');
        ...
        cy.get('[data-cy="top-input-search"]').clear({force : true}).type('Combo Deal Edit');
        cy.wait(1000);

        cy.get('.v-switch-label:first').should('contain','ACTIVE');

        // Check new name
        cy.get('[data-cy$=title]:first').should('contain','Combo Deal Edit');
