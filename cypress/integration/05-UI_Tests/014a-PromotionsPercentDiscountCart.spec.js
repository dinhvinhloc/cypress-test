describe('014a - Setup Promotion - Percentage off of Cart', function () 
{
    it('Percentage off of Cart', function()
    {
        ...
        // Select Point Perk
        cy.get('[data-cy="list-item-new-promotion-6-btn-next"]').click();

        // Name and describe perk
        cy.get('[data-cy="promotion-name"]').clear({force : true}).type('Percent Discount on Cart');
        cy.get('[data-cy="promotion-description"]').clear({force : true}).type('Get a percentage off at check out.');
        ...
        // Set Benefit Discount percent amount
        cy.get('[data-cy="promotion-discount-percent-input-num"]').clear({force : true}).type('10');
        ...
        // Set coupon for perk
        cy.get('[data-cy="promotion-coupon-code-input"]').clear({force : true}).type('KLDSG9S');
        ...
        cy.get('[data-cy="top-input-search"]').type('Percent Discount on Cart')

        cy.get('[data-cy$=title]:first').should('contain','Percent Discount on Cart');
        cy.get('.v-switch-label:first').should('contain','ACTIVE');
        cy.get('.mb-1.text-muted:first').should('contain', 'Get a percentage off at check out.');
