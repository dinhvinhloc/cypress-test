describe('017f - Edit Promotion - Fixed Discount on Cart', function () 
{
    it('Fixed Discount on Cart', function()
    {
        ...
        cy.get('[data-cy="top-input-search"]').clear({force : true}).type('Fixed Amount Discount on Cart');
        ...
        cy.get('[data-cy$=toggle-btn-status]:first').click().wait(1000);
        ...
        cy.get('[data-cy="promotion-name"]').clear({force : true}).type('Fixed Discount on Cart Edit');
        ...
        cy.get('[data-cy="top-input-search"]').clear({force : true}).type('Fixed Discount on Cart Edit');
        cy.wait(1000);

        cy.get('.v-switch-label:first').should('contain','ACTIVE');

        // Check new name
        cy.get('[data-cy$=title]:first').should('contain','Fixed Discount on Cart Edit');
