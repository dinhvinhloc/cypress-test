describe('017c - Edit Promotion Free Item', function () 
{
    it('Edit Free Item', function()
    {
        // Login
        ...
        cy.get('[data-cy="top-input-search"]').clear({force : true}).type('Free Item Automatic Test');
        ...
        cy.get('[data-cy="promotion-name"]').clear({force : true}).type('Free Item Automatic Test Edit');
        ...
        cy.get('[data-cy="top-input-search"]').clear({force : true}).type('Free Item Automatic Test Edit');
        cy.wait(1000);

        cy.get('.v-switch-label:first').should('contain','ACTIVE');

        // Check new name
        cy.get('[data-cy$=title]:first').should('contain','Free Item Automatic Test Edit');
