describe('016a - Setup Promotion - Bundle Deal', function () 
{
    it('Bundle Deal', function()
    {
        // Login
        ...
        // Select Point Perk
        cy.get('[data-cy="list-item-new-promotion-8-btn-next"]').click();

        // Name and describe perk
        cy.get('[data-cy="promotion-name"]').clear({force : true}).type('Bundle Deal');
        cy.get('[data-cy="promotion-description"]').clear({force : true}).type('Bundle Deal.');
        ...
        //cy.get('[data-cy^="promotion-products-required-1-tree-select-selectable-item"]:first').click();
        ...
        // Set Flat Price
        cy.get('[data-cy="promotion-discount-amount-input-num"]').clear({force : true}).type('20');
        ...
        // Set coupon for perk
        cy.get('[data-cy="promotion-coupon-code-input"]').clear({force : true}).type('Z1X2C3');
        ...
        cy.get('[data-cy="top-input-search"]').type('Bundle Deal')

        cy.get('[data-cy$=title]:first').should('contain','Bundle Deal');
        cy.get('.v-switch-label:first').should('contain','ACTIVE');
        cy.get('.mb-1.text-muted:first').should('contain', 'Bundle Deal.');
