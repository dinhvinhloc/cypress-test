describe('API022c - Merchant Point test - order is Rejected', function () {
    it('Test that merchant point will not increase if the order is rejected', function () {
        ...
        cy.waitUntil(() => cy.getMerchantPoints().then((response)=>{
            ...
            console.log("MERCHANT POINT AFTER REJECTED")
            console.log(response)
        }));
        ...
        cy.waitUntil(() => cy.getUserPoints().then((response)=>{
            cy.wrap(response).each((userPoint)=>{
                ...
                // TO DO: Check the Global Point increament later
                else if (userPoint.merchantId == null){
