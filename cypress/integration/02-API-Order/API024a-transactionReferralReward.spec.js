describe('API024a - Test transactional referral reward based on 3 levels', function () {
    it('Test that the referrers at each leavel will get global point reward when referree complete an order', function () {
        ...
        cy.waitUntil(() => cy.getGlobalPoint().then((response) => {
            // console.log(response)
            expect(response).to.equal(user3GlobalPoint + 11380);
        }));
        cy.waitUntil(() => cy.getGlobalPointHistorySummary().then((response) => {
            console.log("user0003 Points")
            console.log(response)
            expect(response.availablePoints).to.equal(user3GlobalPointSummary.availablePoints + 11380)
            expect(response.redeemedPoints).to.equal(user3GlobalPointSummary.redeemedPoints)
        ...
        cy.waitUntil(() => cy.getGlobalPoint().then((response) => {
            // console.log(response)
            expect(response).to.equal(user2GlobalPoint + 1138);
        }));
        cy.waitUntil(() => cy.getGlobalPointHistorySummary().then((response) => {
            console.log("user0002 Points")
            console.log(response)
            cy.wrap(response.transactionReferralReward).each((transaction) => {
                if (transaction.level == 1) {
                    expect(transaction.globalPoints).to.equal(user2GlobalPointSummary.pointsAtLevel[0] == undefined ? 0 + 1138 : user2GlobalPointSummary.pointsAtLevel[0] + 1138)
                } else if (transaction.level == 2) {
        ...
        cy.waitUntil(() => cy.getGlobalPoint().then((response) => {
            // console.log(response)
            expect(response).to.equal(user1GlobalPoint + 113);
        }));
        cy.waitUntil(() => cy.getGlobalPointHistorySummary().then((response) => {
            console.log("user0001 Points")
            console.log(response)
            cy.wrap(response.transactionReferralReward).each((transaction) => {
                if (transaction.level == 1) {
                    ...
                    expect(transaction.globalPoints).to.equal(user1GlobalPointSummary.pointsAtLevel[1] == undefined ? 0 + 113 : user1GlobalPointSummary.pointsAtLevel[1] + 113)
                } else if (transaction.level == 3) {
                }
            })
        }));
        ...
        cy.waitUntil(() => cy.getGlobalPoint().then((response) => {
            // console.log(response)
            expect(response).to.equal(userxGlobalPoint + 11);
        }));
        cy.waitUntil(() => cy.getGlobalPointHistorySummary().then((response) => {
            ...
            cy.wrap(response.transactionReferralReward).each((transaction) => {
                if (transaction.level == 1) {
                    ...
                    expect(transaction.globalPoints).to.equal(userxGlobalPointSummary.pointsAtLevel[2] == undefined ? 0 + 11 : userxGlobalPointSummary.pointsAtLevel[2] + 11)
                }
            })
        }));
