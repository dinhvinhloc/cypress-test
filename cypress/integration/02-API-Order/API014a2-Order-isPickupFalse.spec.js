describe('API014a2 - Test the Order in case confirmation with isPickup = false', function () {
    it('Verify the Order in case confirmation with isPickup = false that merchant allow customer to Dine In', function () {
        ...
        cy.waitUntil(() => cy.merchantOrderCommand(order.id, store1.storeID.id,"complete","").then((response)=>{
            console.log(response);
            expect(response).to.have.property('serveStatus', 'SERVED');
