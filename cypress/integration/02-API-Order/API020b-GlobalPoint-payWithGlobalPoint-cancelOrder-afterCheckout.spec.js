describe('API020b - Global Point test - order canceled after checkout', function () {
    it('Test the case where order is canceled (after checkout) by user and Global Point returned', function () {
        ...
        var store1
        const now = Cypress.dayjs().add(1, 'minutes').format('YYYY-MM-DDTHH:mm:ss.SSSZ');
        ...
        // preparing the order
        cy.waitUntil(() => cy.merchantOrderCommand(order.id, store1.storeID.id, "preparing", { processingTime: 15 }));
        ...
        cy.waitUntil(() => cy.cancelOrder(order2.id).then((response) => {
            console.log(response);
        ...
        cy.waitUntil(() => cy.getGlobalPointHistory().then((response) => {
            ...
            //   expect(firstHistory).to.have.property('globalPoints', 10 * (order.totalAmount - order.taxAmount ));
            expect(firstHistory).to.have.property('globalPoints', Math.trunc(10 * (order.subAmount-order.discountAmount)));
