describe('API012h5 - Percent Discount on Item - highlightAmount', function () {
    it('Test the Percent Discount on Item it is suggested based on highlightAmount', function () {
        ...
        // Set the highlightAmount to 40.99 and requiredAmount to 90
        cy.wrap(deal).then(()=>{
            deal.highlightAmount = 40.99
            deal.requiredAmount = 90
            deal.isAddonCharged = false
            cy.updateDeal(deal,deal.id).then((response)=>{
                expect(response).to.have.property("highlightAmount",40.99)
                expect(response).to.have.property("requiredAmount",90)
        ...
        // Add 1 PG2P2 to the cart which still under limit of highlightAmount, promotion should not be suggested
        var rewardItem
        ...
        // Update the item qty to reach the highlightAmount ($40.99), promotion is suggested
        cy.waitUntil(() => cy.updateItemInCart(1,store1.storeID.id,store1.PG2P2.id,2,store1.AG2AO1.id,1).then((response)=>{
            console.log(response);
            expect(response.appliedPromotion).to.be.an('array').that.is.empty;
        ...
        // Update the item qty to reach the requiredAmount ($90), promotion is applied
        cy.waitUntil(() => cy.updateItemInCart(1,store1.storeID.id,store1.PG2P2.id,3,store1.AG2AO1.id,1).then((response)=>{
            console.log(response);
            // expect(response.appliedPromotion).to.be.an('array').that.is.empty;
            expect(response.appliedPromotion[0]).to.have.property('id', deal.id);  
