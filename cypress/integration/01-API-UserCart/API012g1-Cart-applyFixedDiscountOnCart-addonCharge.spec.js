describe('API012g1 - Fixed Discount on Cart  - Free Addon', function () {
    it('Test the Fixed Discount on Cart where isAddonCharge = false', function () {
        ...
        // Add 5 PG3P5 to the cart and expect fixed discount on Cart deal to be applied
        cy.waitUntil(() => cy.addItemToCart(store1.storeID.id,store1.PG3P5.id,3,store1.AG3AO1.id,1).then((response)=>{
            console.log(response);
            expect(response.appliedPromotion[0]).to.have.property('id', deal.id);  
            expect(response).to.have.property('discountAmount', 34.862);
                        // Check Discounts
        ...
        // Update the cart to lower the cart limit, promotion should be removed.
