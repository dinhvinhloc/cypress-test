describe('API012f5 - Percent Discount on Cart - highlightAmount = true', function () {
    it('Test the Percent Discount on Cart it is suggested based on highlightAmount', function () {
        ...
        cy.wrap(deal).then(()=>{
            deal.highlightAmount = 200
            cy.updateDeal(deal,deal.id).then((response)=>{
                expect(response).to.have.property("highlightAmount",200)
            })
        })
        ...
        var rewardItem
        cy.waitUntil(() => cy.addItemToCart(store1.storeID.id,store1.PG3P5.id,2,store1.AG3AO1.id,1).then((response)=>{
            console.log(response);
        ...
        // Update the item qty to reach the threshold ($200), promotion is suggested
        ...
        // Update the item qty to meet promotion requirement ($350), promotion is applied
        cy.waitUntil(() => cy.updateItemInCart(1,store1.storeID.id,store1.PG3P5.id,5,store1.AG3AO1.id,1).then((response)=>{
            console.log(response);
            expect(response.suggestedPromotion).to.be.an('array').that.is.empty;
