describe('API012h7 - Free Item - highlightAmount', function () {
    it('Test the suggestion of Free Item promotion based on highlightAmount', function () {
        ...
        // SCENARIO 1:
        // requiredAmount = 0; highlightAmount = null
        // The Free Item is applied straight and presented in toBeAppliedPromotion
        cy.wrap(deal).then(()=>{
            console.log("SCENARIO 1")
            deal.highlightAmount = null
            deal.requiredAmount = 0
            deal.isAddonCharged = false
            cy.updateDeal(deal,deal.id).then((response)=>{
                expect(response).to.have.property("highlightAmount",null)
                expect(response).to.have.property("requiredAmount",0)
        ...
        cy.waitUntil(() => cy.updateItemInCart(1,store1.storeID.id,store1.PG2P3.id,2,store1.AG2AO1.id,1).then((response)=>{
            console.log(response);
            // expect(response.appliedPromotion).to.be.an('array').that.is.empty;
            ...
            // expect(response).to.have.property('discountAmount', 13);
                        // Check Discounts
            cy.assertDiscount(response.items,response.discountAmount);
            // Check Tax
            cy.assertAmount(response.appliedTaxes,response.subAmount,response.discountAmount, response.totalAmount,response.serviceFee)
            ...
            let IDList = response.toBeAppliedPromotion.map(a => a.id);
        ...
        // SCENARIO 2:
        // requiredAmount = 250; highlightAmount = null
        // The Free Item will not be suggested
        ...
        cy.wrap(deal).then(()=>{
            ...
            deal.highlightAmount = null
            deal.requiredAmount = 250
            deal.isAddonCharged = false
            cy.updateDeal(deal,deal.id).then((response)=>{
                expect(response).to.have.property("highlightAmount",null)
                expect(response).to.have.property("requiredAmount",250)
                expect(response).to.have.property("isAddonCharged",false)
        ...
        // SCENARIO 3:
        // requiredAmount = 250; highlightAmount = 55.99
        // The Free Item is suggested onlt if cart become higher than highlightAmount
        ...
        cy.wrap(deal).then(()=>{
            ...
            deal.highlightAmount = 55.99
            deal.requiredAmount = 250
            deal.isAddonCharged = false
            cy.updateDeal(deal,deal.id).then((response)=>{
                expect(response).to.have.property("highlightAmount",55.99)
                expect(response).to.have.property("requiredAmount",250)
                expect(response).to.have.property("isAddonCharged",false)
