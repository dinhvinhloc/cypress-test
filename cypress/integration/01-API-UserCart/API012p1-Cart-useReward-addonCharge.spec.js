describe('API012p - Use Reward Item Redeem - isAddonCharge = false', function () {
    it('Business Category: Checking Redeem Item Functionality - isAddonCharge = false', function () {
        ...
        // Add only the Redeem Reward Item into the cart
        // Expect the totalAmount is 0 since addon is not charged
        cy.waitUntil(() => cy.addRedeemItemToCart(store1.storeID.id,deal.id,redeemProductList).then((response)=>{
            ...
            expect(response.totalAmount).to.equal(0);
        }));
        ...
        // Confirm the order but expect clientSecret to be null since the totalAmount is 0

        cy.waitUntil(() => cy.confirmOrder(redeemOrder.id,confirmationInfo).then((response)=>{
            confirmResponse = response
            expect(response.clientSecret).to.equal(null);
        }));
