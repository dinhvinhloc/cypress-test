describe('API012d4 - Combo Deal test - isNewClient = true', function () {
    it('Test that the Combo Deal is not suggested or applied if isNewClient = true AND client has had order', function () {
        ...
        cy.waitUntil(() => cy.addItemToCart(store1.storeID.id,store1.PG1P1.id,1,store1.AG1AO1.id,1).then((response)=>{
            ...
            // Check suggested Promotion SHOULD BE BLANK https://intransigense.atlassian.net/browse/BP-1503
        ...
        cy.waitUntil(() => cy.addItemToCart(store1.storeID.id,store1.PG3P1.id,2,null,null).then((response)=>{
            ...
            // https://intransigense.atlassian.net/browse/BP-1503
